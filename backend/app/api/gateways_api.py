...
async def list_routers(db: DBSession, _: CurrentUser) -> list:
    routers = db.query(Gateway).filter(Gateway.activo == True).order_by(Gateway.nombre).all()
    result = []
    for r in routers:
...
def test_unsaved_router_connection(
    ...
    try:
        with router_pool.connect_to(temp_gateway) as api_conn:
...
async def get_router(gateway_id: uuid.UUID, db: DBSession, _: CurrentUser) -> dict:
...
async def get_router_status(gateway_id: uuid.UUID, db: DBSession, _: AdminOrTecnico) -> GatewayStatus:
    """
...
def test_router_connection(gateway_id: uuid.UUID, db: DBSession, _: AdminOnly) -> GatewayTestResult:
    """
...
def get_router_queues(gateway_id: uuid.UUID, db: DBSession, _: AdminOrTecnico) -> list[dict]:
    """
...
def get_parent_queue(gateway_id: uuid.UUID, db: DBSession, _: AdminOrTecnico) -> dict:
    """
...
def sync_router_pppoe_profiles(gateway_id: uuid.UUID, db: DBSession, _: AdminOnly) -> dict:
    """
...
def get_router_pppoe_profiles(gateway_id: uuid.UUID, db: DBSession, _: AdminOrTecnico) -> list:
    """
    ...
    return db.query(PPPoEProfile).filter(PPPoEProfile.gateway_id == gateway_id).order_by(PPPoEProfile.nombre).all()


@router.get("/{gateway_id}/pppoe-sessions", response_model=list[PPPoESessionActive])
def get_router_pppoe_sessions(gateway_id: uuid.UUID, db: DBSession, _: AdminOrTecnico) -> list:
    """
