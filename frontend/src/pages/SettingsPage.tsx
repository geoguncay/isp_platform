...
export function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'mikrotik' | 'company' | 'billing' | 'suspension' | 'payment_methods' | 'users' | 'alerts'>('mikrotik')
  ...
  type TabType = 'mikrotik' | 'company' | 'billing' | 'suspension' | 'payment_methods' | 'users' | 'alerts'
  type NavItem = { id: TabType; icon: React.ComponentType<{ className?: string }>; label: string }
  const navGroups: { label: string; items: NavItem[] }[] = [
    {
      label: 'Sistema',
      items: [
        { id: 'mikrotik', icon: Router, label: 'MikroTik por Defecto' },
    ...
    },
    {
      label: 'Facturación',
      items: [
        { id: 'billing', icon: Receipt, label: 'Configuración de Cobros' },
        { id: 'suspension', icon: Ban, label: 'Políticas de Suspensión' },
        { id: 'payment_methods', icon: CreditCard, label: 'Métodos de Pago' },
      ],
    },
    {
      label: 'Plataforma',
      items: [
        { id: 'users' as const, icon: Users, label: 'Operadores y Usuarios' },
        { id: 'alerts' as const, icon: Bell, label: 'Alertas del Sistema' },
      ],
    },
  ]
  ...
  return (
    <div className="animate-fade-in">
      {/* Page header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">Ajustes del WISP</h1>
        <p className="text-muted-foreground text-sm mt-0.5">
          Configura los parámetros globales de tu plataforma: red, facturación, empresa, usuarios y alertas.
        </p>
      </div>
      ...
      <div className="flex gap-6 items-start">
        ...
        <aside className="w-56 flex-shrink-0 sticky top-6">
          <nav className="glass-card p-2 space-y-4">
            {navGroups.map((group) => (
              <div key={group.label}>
